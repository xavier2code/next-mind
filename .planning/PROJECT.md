...
**v1.3 进行中** — 容器化环境搭建、全量回归验证、自动化测试补充。

## Current Milestone: v1.3 全量回归验证

**Goal:** 搭建 Docker 容器化验证环境，对 v1.0-v1.2 所有功能进行全量回归验证，产出验证报告并修复发现的问题。

**Target features:**
- Docker 容器化环境（Dockerfile + docker-compose，PostgreSQL + Next.js app）
- v1.0-v1.2 全功能回归验证（手动走查 + 自动化测试）
- 验证报告（每里程碑 PASS/FAIL 清单）
- 发现问题的修复

## Current State

**Shipped:** v1.2 文件处理 (2026-03-27)
**In Progress:** v1.3 全量回归验证
...
参见 Current Milestone: v1.3 全量回归验证。
...
- ✓ Docker 容器化开发/测试环境 — v1.3 Phase 11
- ✓ 测试基础设施（Playwright配置、Auth fixtures、LLM mock、Health endpoint） — v1.3 Phase 12

### Active

- [ ] v1.0 全功能回归验证（认证、LLM Gateway、MCP、Bash工具、Skills、审批）
- [ ] v1.1 全功能回归验证（Agent系统、任务分解、调度、通信、控制、展示）
- [ ] v1.2 全功能回归验证（存储、上传、提取、管理、对话集成）
- [ ] 补充自动化测试（单元测试 + E2E 测试）
- [ ] 修复验证中发现的问题
...
---
*Last updated: 2026-03-27 after Phase 12 test-infrastructure completion*
